...
const Acadamic = () => {
  ...
  const [validateAcForm, setValidateAcForm] = useState("");
 const{
    Acslide, setAcSlide,
    hgname, setHgnamel,
    ...
    yearLevel, degreeProgram, email, permanentAddress,
    phoneNumber, birthMonth, emrfirstName,
    birthDay, birthYear, countries, gender, setGender,
    matric, setMatric, extra, setExtra, scrollRef,newStudent,
    isAcadamicFormValid, setAcadamicFormValid,SubmitLoader, setSubmitLoader,message, setMessage 
  } = useUser();

  const handleSlide = () => setAcSlide(prev => !prev);

  const Filled = hgname?.trim() && transcript && matric && extra?.trim();
  ...
  const handleTranscriptUpload = (e) => {
    ...
    reader.onloadend = () => {
      setTimeout(() => {
        ...
        setListTranscript(prev => [file, ...prev]);
  ...
  return (
    <div className='border-b border-b-gray-300'>
      ...
      <div className={`transition-all duration-500 overflow-hidden ${Acslide ? 'max-h-[4000px] mt-6 px-6 overflow-y-auto' : 'max-h-0 px-6'}`} ref={scrollRef}>
        {Acslide && (
          <>
            {/* High School Name */}
            ...
            <div className='flex flex-wrap gap-10 mt-6'>
              ...
              <div>
                ...
                <div className="relative w-64 h-64 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer overflow-hidden">
                  <input type="file" accept="image/*" onChange={handleTranscriptUpload} required
                  ...
                  <div className="w-full h-full relative">
                    ...
                    {transcriptLoader && <div className="flex justify-center items-center h-full"><FileLoader /></div>}
                    {!transcriptLoader && showTranscript && transcriptPreview && (
                      <img src={transcriptPreview} alt="Uploaded Transcript Preview" className="w-full h-full object-cover object-center" />
                    )}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="relative w-64 h-64 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer overflow-hidden">
                  <input type="file" accept="image/*" onChange={handleMatricUpload} required
                    className="absolute w-64 h-64 top-10 z-10 opacity-0 focus:outline-none hover:ring-2 hover:ring-purple-500 cursor-pointer" />
                  <div className="w-full h-full relative">
                    ...
                    {matricLoader && <div className="flex justify-center items-center h-full"><FileLoader /></div>}
                    {!matricLoader && showMatric && matricPreview && (
                      <img src={matricPreview} alt="Uploaded Matric Result Preview" className="w-full h-full object-cover object-center" />
                    )}
                  </div>
                </div>
            ...
            <div className='flex flex-col flex-wrap gap-2 pt-10 border-b border-b-gray-200'>
              ...
              <div className="flex flex-col gap-5 mt-3 mb-10">
                {["Student Council", "Club Organization", "Basketball"].map((val, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    <input value={val} onChange={handleExtra} type="radio" name="radiobtn" className="w-5 h-5 accent-[#8200DB]" id={val} />
                    <label htmlFor={val} className="text-gray-800">{val}</label>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className='flex justify-between mr-20 mt-5'>
              <div>
                {isAcadamicFormValid ? (
                  <p className="text-green-600 font-semibold mt-2">{validateAcForm}</p>
                ) : (
                  <p className="text-red-600 font-semibold mt-2">{validateAcForm}</p>
                )}
              
              </div>
               {!Filled ? <>
               
                 <button onClick={handleSubmitbtn} className='flex items-center justify-center text-white cursor-pointer w-42 h-10 bg-[#AF89EA] hover:bg-[#8200DB] hover:font-bold hover:text-white rounded-xl transition-all duration-800 mt-5 mb-5'>
                 Submit
              </button>

               </> :<>
                 <Link to='/registrationpayment' className='flex items-center justify-center text-white cursor-pointer w-42 h-10 bg-[#AF89EA] hover:bg-[#8200DB] hover:font-bold hover:text-white rounded-xl transition-all duration-800 mt-5 mb-5'>
                 Go to Payment
              </Link>
